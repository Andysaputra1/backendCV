...
MODEL_FILENAME = "best_mushroom_cnn.keras"
MODEL_PATH = os.path.join(BASE_DIR, MODEL_FILENAME)
SCALER_PATH = os.path.join(BASE_DIR, "mushroom_scaler_2.joblib")
LE_PATH = os.path.join(BASE_DIR, "mushroom_le_2.joblib")

# === BAGIAN PENGGABUNGAN FILE (MAGIC HAPPENS HERE) ===
def join_model_parts(output_path):
    # Cek apakah model utuh sudah ada? Kalau ada, skip.
    if os.path.exists(output_path):
        return

    print("Menggabungkan pecahan model...")
    parts = []
    # Cari semua file yang berakhiran .part0, .part1, dst
    for filename in sorted(os.listdir(BASE_DIR)):
        if filename.startswith(MODEL_FILENAME) and ".part" in filename:
            parts.append(os.path.join(BASE_DIR, filename))
    
    if not parts:
        print("Error: Tidak ditemukan pecahan file model (.part)!")
        return

    # Gabungkan file
    with open(output_path, 'wb') as outfile:
        for part in sorted(parts): # Pastikan urutan part0, part1, dst benar
            print(f"Menggabungkan {part}...")
            with open(part, 'rb') as infile:
                outfile.write(infile.read())
    print(f"Model berhasil digabung menjadi: {output_path}")

# Panggil fungsi penggabung sebelum load model
join_model_parts(MODEL_PATH)
# ======================================================
...
def extract_features(image):
    try:
        ...
        if image.shape[:2] != IMG_SIZE: image = cv2.resize(image, IMG_SIZE)
        ...
        features_hog = hog(gray, orientations=9, pixels_per_cell=HOG_PIXELS_PER_CELL, cells_per_block=HOG_CELLS_PER_BLOCK, visualize=False, block_norm='L2-Hys', feature_vector=True)
        radius = 1; n_points = 8 * radius
        lbp = local_binary_pattern(gray, n_points, radius, method='uniform')
        (hist_lbp, _) = np.histogram(lbp.ravel(), bins=np.arange(0, n_points + 3), range=(0, n_points + 2))
        hist_lbp = hist_lbp.astype("float"); hist_lbp /= (hist_lbp.sum() + 1e-7)
...
def predict():
    ...
    try:
        ...
        return jsonify({"name": class_name, "confidence": confidence, "desc": info["desc"], "region": info["region"], "edibility": info["edibility"]})
    except Exception as e:
